...
class ReduceNoise():
    ...
    def PrepareData4Saving(self,CISsavgolWindow):

 
        Data385 = -self.RawData[1]*PixelSize_um

        # Data385[1]=Data385[1]-Data385[1][0]

        y = savgol_filter(Data385, CISsavgolWindow, SvGolPol)
...
class plotPlotly(CIScurveFromImage):
    ...
    def PlotCIS385_12k(self, MaxWaveWindow, StpWindowSize,SvGolPol,cis_original,padded_valuesList):
        fig = go.Figure()
        ...
        fig.add_trace(
            go.Scatter(y=list(self.ydb), line_color='red',
                       name='Optimized Curvature Calculated'))

   
        fig.add_trace(
            go.Scatter(y=list(cis_original), line_color='Magenta',
                       name='Original curvature'))
        
        fig.add_trace(
            go.Scatter(y=padded_valuesList, line_color='Orange',
                       name='Remaining CURVE'))
...
class Remaining_Curvature():
    def __init__(self,FrontBack):
        self.FrontBack =FrontBack
        
    def CreatePaddedValues(self,values):
        ...
        if len(values)<385:
            ...
            ContinueCalc = float(simpledialog.askstring(
                "Input", FrontBack+' length ='+str(len(values))+" :There are less with 385 points! continue? Yes=1\\No=0:", parent=root))
            print('Done')
            print('**************************************************************************')
            l= 385 - len(values)
        ...
        return ContinueCalc,values,padded_values
...
while (1):
    ...
    FrontBack='Front'
    valuesFront =CorrectionArrFRONT

    ContinueCalc,valuesFront,padded_valuesFront= Remaining_Curvature(FrontBack).CreatePaddedValues(valuesFront)
    if not ContinueCalc:
        continue
    ...
    try:
        ...
        ContinueCalc,valuesBack,padded_valuesBack= Remaining_Curvature(FrontBack).CreatePaddedValues(valuesBack)
        if not ContinueCalc:
            continue
    except:
        1
    ...
    # plt.figure()
    # # plt.plot(cisFRONTold)
    # plt.plot(curveData_385)
    ...
    if not len(cisFRONTold) == 0:
        FrontBack='Front'
        curveData_385=-pd.DataFrame(cisFRONTold)[0]/PixelSize_um
        curveData_385 = pd.concat([curveData_385, pd.Series(curveData_385[len(curveData_385)-1])])
        # curveData_385.append(pd.Series(curveData_385[len(curveData_385)-1]))
        curveData_385 = curveData_385.reset_index(drop=True)
        curveData_385 = curveData_385.reset_index()
        
        curveData_385.rename(columns={0: 1}, inplace=True)
        curveData_385.rename(columns={'index': 0}, inplace=True)

        xdb = curveData_385[0]
        ydb = curveData_385[1]+  pd.DataFrame(valuesFront)[0] / PixelSize_um-curveData_385[1][0]
        
        cis_original=curveData_385[1]
        
        valuesList=list(pd.DataFrame(valuesFront)[0] / PixelSize_um)
        
        plotTitle = pthF+'-->'+FrontBack+" 385 points - For CIS (for implamentation) Slider switched to Step: "  # Can modify Plot title
        fileName =FrontBack+ " Optimized Curvature 385" + ".html"
        tlt12k=0
        z12k=0
        figCIScalc = plotPlotly(0, plotTitle, fileName, RecDimX, RecDimY, xdb,
                                ydb, tlt12k, z12k).PlotCIS385_12k(MaxWaveWindow, StpWindowSize,SvGolPol,cis_original,valuesList)
        print('**************************************************************************')
        ...
        CIS_optimized385=pd.DataFrame({0:xdb,1:ydb}) 
        
        FileNameCSV =FrontBack+  ' Optimized Curvature ' +MachineName+ '_385_'+current_date+'.csv'
        y385 = ReduceNoise(CIS_optimized385).PrepareData4Saving(CISsavgolWindow)
    
        CIScurve385= ReduceNoise(CIS_optimized385).SaveCSV(FileNameCSV, y385)

    if not len(cisBACKold) == 0:
        FrontBack='Back'
        curveData_385=-pd.DataFrame(cisBACKold)[0]/PixelSize_um
        curveData_385 = pd.concat([curveData_385, pd.Series(curveData_385[len(curveData_385)-1])])
        # curveData_385.append(pd.Series(curveData_385[len(curveData_385)-1]))
        curveData_385 = curveData_385.reset_index(drop=True)
        curveData_385 = curveData_385.reset_index()
        ...
        xdb = curveData_385[0]
        ydb = curveData_385[1]+  pd.DataFrame(valuesBack)[0] / PixelSize_um-curveData_385[1][0]
        
        cis_original=curveData_385[1]
        
        valuesList=list(pd.DataFrame(valuesBack)[0] / PixelSize_um)
        ...
        CIScurve385= ReduceNoise(CIS_optimized385).SaveCSV(FileNameCSV, y385)
        # curveData_385 = pd.concat([curveData_385, curveData_385[len(curveData_385)-1]])
        
    if not len(cisFRONTnew) == 0:
        # RawData_12k=cisFRONTnew
        FrontBack='Front'
        ...
        RawData_12k.rename(columns={0: 1}, inplace=True)
        RawData_12k.rename(columns={'index': 0}, inplace=True)
        ...
        plotTitle = pthF+'-->'+FrontBack+" _12k points - For CIS (for implamentation) Slider switched to Step: "  # Can modify Plot title
