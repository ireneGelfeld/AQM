...
#    if os.path.isfile(pthF+folder[i]+'/Front/RawResults/'+'Registration_Left.csv'):
#        try:
#            RegLeft=pd.read_csv(pthF+folder[i]+'/Front/RawResults/'+'Registration_Left.csv',usecols=[4])
#            break;
#        except:
#            continue
#
#lclr=RegLeft.iloc[:,0].unique().tolist()
#lclr.sort()
#print('Select Number for color:\n')
#for i,c in enumerate(lclr):
#    print(c+'-'+str(i)+'\n');
#
#clrNumber = int(input("Insert Color Number: "))
#setNumber=1;    
...
# FaltNumberSer_FRONT=CalcC2C_AvrgOfAll(pthF,folder,'Front',JobLength,PanelLengthInMM,'Right').FlatNumberSerialized();

# try:
#     DataPivotBack,flatNumberFailedBack=CalcC2C_AvrgOfAll(pthF,folder,'Back',JobLength,PanelLengthInMM,'Left').CalcC2CregForLeftRight();
    
#     FaltNumberSer_BACK=CalcC2C_AvrgOfAll(pthF,folder,'Back',JobLength,PanelLengthInMM,'Right').FlatNumberSerialized();
