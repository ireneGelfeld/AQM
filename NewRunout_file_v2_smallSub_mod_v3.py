...
def PlotRegistrationMapForPnls(ClrDF_fromTargetS_goly_allPanels,colorInUseName,clrToUse,pnelToShow,NumOfSecToShow,sInput,fileNME,scaling_factor):
    ...
    for Pnl in pnelToShow:
        ...
        for sec in range(NumOfSecToShow):
            ...
            mirrored_result = mirror_signal_along_x_axis(ll,scaling_factor)
            secPos=np.mean(ClrDF_rawXY_allPanels[Pnl][sec]['Magenta_x'])
            # mirrored_result=list(C2Cmat_allPanels[Pnl][0]-1)
            ll1=list(np.asarray(ll)*scaling_factor+secPos)
            ...
            for res in result:
                ...
                fig.add_trace(go.Scatter(y=list(rnge[i:res]),x=list(ll1[i:res]), line=dict(color=clr1,width=2) ,mode='lines' ,text=string_list,
                # font_size=18,
                hoverinfo='text', showlegend=False))
                fig.add_trace(go.Scatter(y=list(rnge[i:res]),x=list(mirrored_result1[i:res]), line=dict(color=clr2,width=3),mode='lines',text=string_list,
                # font_size=18,
                hoverinfo='text', showlegend=False))
                i=res-1
...
def mirror_signal_along_x_axis(signal,scaling_factor):
    mirrored_signal = [-value*scaling_factor for value in signal]
...
RefC03='Orange'
if RefC03 in colorInUseName:
    col1_col2_Sgoly_allPanels_continues = createContinues(col2_col3_Sgoly_allPanels_continues,col2_col3_Sgoly_allPanels,len(ImRoi.keys()),PointsToIgnore);
...
pnelToShow=sInputListSORTED

NumOfSecToShow=NumOfSec


PlotRegistrationMapForPnls(ClrDF_fromTargetS_goly_allPanels,colorInUseName,clrToUseInJOB,pnelToShow,NumOfSecToShow,sInput,fileNME,scaling_factor)
