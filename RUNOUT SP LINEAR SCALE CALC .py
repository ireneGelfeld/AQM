...
def mirror_signal_along_x_axis(signal):
    mirrored_signal = [-value for value in signal]
    return mirrored_signal
...
def Plot_Panel_number(fig,ymax,indexPanelNameDic):
    ...
    for key, value in indexPanelNameDic.items():
         if (int(value.split('-')[-1])-pValue)>2:
             colorFlag=abs(colorFlag-1)
         pValue=int(value.split('-')[-1])
         fig.add_trace(go.Scatter(x=[key], y=[ymax],
         ...
         fig.data[len(fig.data)-1].showlegend = False
         fig.add_vline(x=key, line_width=0.5, line_dash="dash", line_color=greenRedLine[colorFlag])
 
    return fig

def DiffBetween_two_DF(db1,db2):
    ...
    for i in range(3):
        for c in db1[i].columns: 
            Ddb[i][c]=db1[i][c].reset_index(drop=True)  -db2[i][c].reset_index(drop=True) 
            
    return Ddb
            
def Create_indexPanelNameDic(db):
    
    indexPanelNameDic=[]
    for i in range(len(db.keys())):
        indexPanelNameDic.append(439*(i+1))
        
    return indexPanelNameDic
        
def Calc_Median_average_std(Ddb):
    ...
    for i in  range(3):
        for c in Ddb[i].columns:
            Median[i]=np.median((Ddb[i][c]));
            Average[i]=np.mean((Ddb[i][c]))
            std[i]=np.std((Ddb[i][c]))
        
        
    for i in range(3):
        MedianAllSide=MedianAllSide + Median[i]
        AverageAllSide=AverageAllSide + Average[i]
        stdAllSide=stdAllSide + std[i]
            
    MedianAllSide= MedianAllSide/3
    AverageAllSide= AverageAllSide/3
    stdAllSide= stdAllSide/3
    
    return Median,Average,std,MedianAllSide,AverageAllSide,stdAllSide
...
def find_nan_coordinates_with_column_names(df):
    nan_coords = [(row, df.columns[col]) for row in range(df.shape[0]) for col in range(df.shape[1]) if pd.isna(df.iloc[row, col])]
    return nan_coords        
#################################################################################################  



# sInput= r'C:\B2_17082023\B2_17082023\PCCimage\Production[250][17-08-2023 11-34-07]'
...
printType= 'without Runout compensation'


    
path_FnameClrDF=sInput+'\ClrDF_rawXY_allPanels.pkl'
with open(path_FnameClrDF, 'rb') as f:
    ClrDF_rawXY_allPanels = pickle.load(f)
...
# C2CmatOP_side = Circles(sInput).calcC2C('Magenta', ClrDF_raw[0]);
    ...
    x={}
    dymeanList=[]
    for i in ClrDF_rawXY.keys():
      dymeanList=Circles(pth).CalcDiffTarget(ClrDF_rawXY[i],dymeanList)  
    ...
    for i in ClrDF_rawXY.keys():
        C2Cmat[i],MaxMinColor[i] = Circles(pth).calcC2C('Magenta', ClrDF_fromTargetS_goly_allPanels[Pnl][i]);
    ...
    RefC01='Cyan'
    Color_Black_Sgoly_allPanels[Pnl]=Color_To_Black_4allPanel(ClrDF_fromTargetS_goly,RefC01,colorInUseName,len(ClrDF_rawXY.keys()))
    # # Cyan_Black_Sgoly={};
    # # RefCl='Cyan'
    # # for i in range(3):
    ...
    RefC02='Magenta'
    Green_Black_Sgoly_allPanels[Pnl]=Color_To_Black_4allPanel(ClrDF_fromTargetS_goly,RefC02,colorInUseName,len(ClrDF_rawXY.keys()))
    Green_Black_NO_FILTER_allPanels[Pnl]=Color_To_Black_4allPanel(ClrDF_fromTarget,RefC02,colorInUseName,len(ClrDF_rawXY.keys()))

    RefC03='Orange'
    if RefC03 in colorInUseName:
        col1_col2_Sgoly_allPanels[Pnl]=Color_To_Black_4allPanel(ClrDF_fromTargetS_goly,RefC03,colorInUseName,len(ClrDF_rawXY.keys()))

    RefC04='Yellow'
    col2_col3_Sgoly_allPanels[Pnl]=Color_To_Black_4allPanel(ClrDF_fromTargetS_goly,RefC04,colorInUseName,len(ClrDF_rawXY.keys()))
    
    RefC05='Black'
    col3_col4_Sgoly_allPanels[Pnl]=Color_To_Black_4allPanel(ClrDF_fromTargetS_goly,RefC05,colorInUseName,len(ClrDF_rawXY.keys()))
    
    RefC06='Blue'
    if RefC06 in colorInUseName:
        col4_col5_Sgoly_allPanels[Pnl]=Color_To_Black_4allPanel(ClrDF_fromTargetS_goly,RefC06,colorInUseName,len(ClrDF_rawXY.keys()))

    RefC07='Green'
    if RefC07 in colorInUseName:
        col5_col6_Sgoly_allPanels[Pnl]=Color_To_Black_4allPanel(ClrDF_fromTargetS_goly,RefC07,colorInUseName,len(ClrDF_rawXY.keys()))
...
############ calc shift
ClrDF_fromTarget_ByPanels={}
panels={}
for i in range(1,12):
    ClrDF_fromTarget_ByPanels[i]=pd.DataFrame(columns=colorInUseName)
    panels[i]=[]

ClrDF_fromTarget_ByPanels_slope={}
for i in range(1,12):
    ClrDF_fromTarget_ByPanels_slope[i]=pd.DataFrame(columns=colorInUseName)
...
for k in ClrDF_fromTargetS_goly_allPanels.keys():
   ...
   if not panel:
       panel=11
   panels[panel].append(k)
   colorsInSec1={}
   colorsInSec1_slope={}
   for clr in colorInUseName:
       colorsInSec1[clr]=ClrDF_fromTargetS_goly_allPanels[k][sec][clr][0]*pixSize
       coefficients = np.polyfit(x*1500, ClrDF_fromTargetS_goly_allPanels[k][sec][clr][list(x)]*pixSize, 1)
       colorsInSec1_slope[clr]=[coefficients]
   ClrDF_fromTarget_ByPanels[panel]=pd.concat([ClrDF_fromTarget_ByPanels[panel],pd.DataFrame(colorsInSec1,index=[0])],axis= 0)    
   ClrDF_fromTarget_ByPanels_slope[panel]=pd.concat([ClrDF_fromTarget_ByPanels_slope[panel],pd.DataFrame(colorsInSec1_slope,index=[0])],axis= 0)    
    
for i in range(1,12):
    ClrDF_fromTarget_ByPanels[i]=ClrDF_fromTarget_ByPanels[i].reset_index(drop=True)
    ClrDF_fromTarget_ByPanels_slope[i]=ClrDF_fromTarget_ByPanels_slope[i].reset_index(drop=True)
    # ClrDF_fromTarget_ByPanels_slope[i]=ClrDF_fromTarget_ByPanels_slope[i][0]*65000



##################################################################
PnlNum=1
for PnlNum in range(1,12):
    fig = go.Figure()
    PlotTitle='Panel '+str(PnlNum)
    fileName=PlotTitle+'.html'
    
    
    
    for j,pnl in enumerate(panels[PnlNum]):
        db=ClrDF_fromTargetS_goly_allPanels[pnl][sec]
        ...
        for c in db.columns:  
            ...
            fig.add_trace(go.Scatter(y=values, line=dict(color=lineColor, dash='solid')  ,name=c+' '+pnl+' line'))
...
ClrDF_fromTargetS_goly_allPanels_continues = createContinues(ClrDF_fromTargetS_goly_allPanels_continues,ClrDF_fromTargetS_goly_allPanels,len(ClrDF_rawXY.keys()),PointsToIgnore);

ClrDF_fromTarget_allPanels_continues = createContinues(ClrDF_fromTarget_allPanels_continues,ClrDF_fromTarget_allPanels,len(ClrDF_rawXY.keys()),PointsToIgnore);

RefC01='Cyan'
RefCl=RefC01
if RefC01 in colorInUseName:
    Color_Black_Sgoly_allPanels_continues = createContinues(Color_Black_Sgoly_allPanels_continues,Color_Black_Sgoly_allPanels,len(ClrDF_rawXY.keys()),PointsToIgnore);

RefC02='Magenta'

RefCl=RefC02
if RefC02 in colorInUseName:
    Green_Black_Sgoly_allPanels_continues = createContinues(Green_Black_Sgoly_allPanels_continues,Green_Black_Sgoly_allPanels,len(ClrDF_rawXY.keys()),PointsToIgnore);

RefC03='Orange'
if RefC03 in colorInUseName:
    col1_col2_Sgoly_allPanels_continues = createContinues(col1_col2_Sgoly_allPanels_continues,col1_col2_Sgoly_allPanels,len(ClrDF_rawXY.keys()),PointsToIgnore);

RefC05='Black'
RefCl=RefC05

if RefC05 in colorInUseName:
    col3_col4_Sgoly_allPanels_continues = createContinues(col3_col4_Sgoly_allPanels_continues,col3_col4_Sgoly_allPanels,len(ClrDF_rawXY.keys()),PointsToIgnore);



RefC04='Yellow'
if RefC04 in colorInUseName:
    col2_col3_Sgoly_allPanels_continues = createContinues(col2_col3_Sgoly_allPanels_continues,col2_col3_Sgoly_allPanels,len(ClrDF_rawXY.keys()),PointsToIgnore);
  
RefC06='Blue'
if RefC06 in colorInUseName:
    col4_col5_Sgoly_allPanels_continues = createContinues(col4_col5_Sgoly_allPanels_continues,col4_col5_Sgoly_allPanels,len(ClrDF_rawXY.keys()),PointsToIgnore);
RefC07='Green'
if RefC07 in colorInUseName:
    col5_col6_Sgoly_allPanels_continues = createContinues(col5_col6_Sgoly_allPanels_continues,col5_col6_Sgoly_allPanels,len(ClrDF_rawXY.keys()),PointsToIgnore);
